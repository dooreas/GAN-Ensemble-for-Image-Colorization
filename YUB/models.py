...
class MainModel(nn.Module):
    ...
    def setup_input(self, data):                                      #Need to change this bit
        self.Y = data['Y'].to(self.device)
        self.uv = data['uv'].to(self.device)

    def forward(self):                                           #Need to change this bit
        self.fake_color = self.net_G(self.Y)

    def backward_D(self):                                         #Need to change this bit
    ...
    def backward_G(self):                                              #Need to change this
        fake_image = torch.cat([self.Y, self.fake_color], dim=1)
